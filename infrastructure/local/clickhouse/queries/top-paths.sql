-- Top 10 URI stems by request count (last 7 days of objects under PREFIX).
-- Placeholders substituted by scripts/query.sh: __S3_URL__, __AWS_KEY__, __AWS_SECRET__
-- CloudFront standard logs are tab-separated and gzipped.

SELECT
  cs_uri_stem AS path,
  count() AS requests,
  countIf(sc_status >= 400) AS errors,
  round(100.0 * countIf(sc_status >= 400) / count(), 2) AS error_rate_pct
FROM s3(
  '__S3_URL__',
  '__AWS_KEY__',
  '__AWS_SECRET__',
